...
<div class="container-fluid" id="coding-area">
	...
	<div class="container" id="question-statement">
		<h2><%=qd.getTitle() %></h2>
		<small class="text-muted">Max score: <%=qd.getMarks() %></small>
		<small class="ml-lg-5">
			<a href="submissions.jsp?q=<%=qd.getId()%>" target="_blank">
				Submissions <i class="fas fa-external-link-alt"></i>
			</a>
		</small>
		<small class="ml-lg-5">
			<a href="editorial.jsp?q=<%=qd.getId()%>" target="_blank">
				Editorial <i class="fas fa-external-link-alt"></i>
			</a>
		</small>
		<small class="ml-lg-5 float-right text-muted">
			Domain rank: <%=ValidateField.getCategoryRank(qd.getCid(), session.getAttribute("username").toString())%>
		</small>
		<hr>
		<div class="container"><%=qd.getStatement() %></div>
	</div>
	<br><br>
	<%
		boolean t1 = qd.getLang().equals("all");
		boolean t2 = qd.getLang().equals("java");
		if(t1 || t2){
			String jnote = t1 ?
	...
	<div class="container" id="back-button">
		...
		<div class="text-success text-center hidden" id="success-message">
			Congratulations! You have earned <%=qd.getMarks()%> marks.
		</div>
		<br>
		<div class="text-right">
			<a href="problems.jsp?cat=<%=qd.getCid()%>&scat=<%=qd.getScid()%>">
...
<input type="hidden" id="allowed-languages" value="<%=qd.getLang() %>">
...
<script type="text/javascript">
    ...
    function execute(command){
		...
		if(command === "run" && $("div#checkbox-div > input[type=checkbox]").prop("checked")){
			...
			var tc_count = command === "run" ? "<%=qd.getTc_public() %>" : "<%=qd.getTc_private() %>";
			$.ajax({
				type: "POST",
				url: "Compile",
				data: {
