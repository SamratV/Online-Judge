...
<div class="container-fluid" id="coding-area">
	...
	<div class="container" id="question-statement">
		<h2><%=tqd.getTitle() %></h2>
		<small>Max score: <%=tqd.getMarks() %></small>
		<small class="ml-lg-5">
			<a href="editorial.jsp?q=<%=tqd.getId()%>" target="_blank">
	...
	function execute(command){
		...
		if(command === "run" && $("div#checkbox-div > input[type=checkbox]").prop("checked")){
			...
			$.ajax({
				type: "POST",
				url: "Compile",
				data: {source: src, lang: lang, action: command, qid: "<%=tqd.getId() %>", tc_count: tc_count},
				success: function(data){
					setTimeout(function(){
