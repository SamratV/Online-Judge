...
<html>
	...
	<body>
		...
		<%
			...
			String qtitle     = qd.getTitle();
			String qstatement = qd.getStatement();
			int qtc_public    = qd.getTc_public();
			int qtc_private   = qd.getTc_private();
			int cid           = qd.getCid();
			int scid          = qd.getScid();
			boolean uploaded  = qd.isUploaded();
	            ...
	            <div class="container-fluid">
					...
					<div class="container col-lg-12">
	                	...
	                	<% if(uploaded){%>
		                	...
		                	<a href="../practice_preview_editor.jsp?q=<%=qid %>" class="btn btn-default" target="_blank">Preview</a>
	                	<% }else{%>
	                ...
	                <div class="container col-lg-12 hidden" id="edit-question">
	                	<form method="post" action="../EQStatement" id="edit-question-form" onsubmit="return checkQuestion();">
							...
							<div class="form-group">
								...
								<select class="form-control" id="qcat" disabled>
									<%
										PreparedStatement stmt = link.prepareStatement("SELECT cid, cname FROM category ORDER BY cname");
										ResultSet result = stmt.executeQuery();
										while(result.next()){
											int id      = result.getInt("cid");
											String name = result.getString("cname");
											if(id == cid) out.println("<option selected>" + name + "</option>");
										}
										stmt.close();
									%>
								</select>
							...
							<div class="form-group">
								<label for="qscat">Sub-category</label>
								<select class="form-control" id="qscat" name="qscat" required="required">
									<%
										stmt = link.prepareStatement("SELECT scid, scname FROM subcategory WHERE cid=? ORDER BY scname");
										stmt.setInt(1, cid);
										result = stmt.executeQuery();
										while(result.next()){
											int id          = result.getInt("scid");
											String name     = result.getString("scname");
											String selected = id == scid ? "selected" : "";
