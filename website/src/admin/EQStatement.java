...
public class EQStatement extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String qscat 	  = request.getParameter("qscat");
			String qstatement = request.getParameter("qstatement");
			
			if(ValidateField.checkQuestion(qtitle, qstatement)) {
				String query = "UPDATE questions SET qtitle=?, qstatement=?, scid=? WHERE qid=?";
				PreparedStatement stmt = link.prepareStatement(query);
				stmt.setString(1, qtitle);
				stmt.setString(2, qstatement);
				stmt.setInt(3, Integer.parseInt(qscat));
				stmt.setInt(4, Integer.parseInt(qid));
				stmt.executeUpdate();
				stmt.close();
			}

			response.sendRedirect("ea/edit_question.jsp?qid=" + qid);
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
